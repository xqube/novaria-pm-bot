...
if (!BOT_TOKEN) throw new Error("Missing BOT_TOKEN in .env file");
...
function htmlEscape(str: string | undefined | null): string {
  return String(str ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
function formatLastSeen(offlineSince: number | null): string {
...
function isSenderOwner(ctx: Context): boolean {
...
bot.command("start", async (ctx) => {
  ...
  if (!state.ownerId) {
    ...
    await saveState();
    await ctx.reply(
      `👋 Hello! You've been registered as the Bot Owner.\n` +
      `🆔 Your ID: ${ctx.from.id}\n\n` +
      `Available commands:\n` +
...
bot.command("online", async (ctx) => {
  ...
  repliedUsers.clear();
  await ctx.reply(
    `🟢 <b>You're now ONLINE</b>\n` +
    `<blockquote>📡 STATUS     : <b>ONLINE</b>\n🤖 RESPONDER : <b>DISABLED</b></blockquote>\n` +
    `Customers will no longer receive automated replies.`,
    { parse_mode: "HTML" }
  );
});
...
bot.command("offline", async (ctx) => {
  ...
  await ctx.reply(
    ...
    `Customers will now receive automated replies.`,
    { parse_mode: "HTML" }
  );
});
...
bot.command("status", async (ctx) => {
  ...
  await ctx.reply(
    ...
    `🤖 RESPONDER : <b>${state.status === "OFFLINE" ? "ACTIVE" : "DISABLED"}</b></blockquote>`,
    { parse_mode: "HTML" }
  );
});
